...
本项目使用 **KAT-Coder-Pro V1** 模型开发，以下是详细的开发提示词：
...
```
优化界面和用户体验：
1. 添加主菜单界面，包含难度选择（简单/普通/困难）
2. 添加游戏说明页面，详细介绍游戏规则和操作
3. 按钮放在画布左右两侧，避免遮挡游戏画面
4. 左侧面板：信息面板、技能按钮、统计面板、成就面板
5. 右侧面板：暂停按钮、模式选择器
6. 使用现代化的渐变背景和半透明面板设计
7. 技能按钮可折叠，节省空间
8. 添加暂停功能，按ESC或空格键暂停
9. 游戏结束界面显示详细统计信息
10. 所有按钮要有hover效果和过渡动画
```
...
```
技术实现细节：
1. 使用递归回溯算法生成完美迷宫，确保所有区域可达，无死路
2. 迷宫生成时，刺应该替换部分墙（walls[y][x] === 1），而不是放在通道上
3. 鬼的AI使用简单寻路算法，选择距离玩家最近的方向移动
4. 粒子系统：收集物品时产生金色粒子，清除鬼时产生红色粒子
5. 粒子具有重力效果（vy += 0.1）和渐隐动画（alpha = life / maxLife）
6. 使用localStorage保存游戏数据：最高分、总游戏数、总时长、总分数、总收集数、总清除鬼数、成就状态
7. 成就检查每5秒执行一次，避免频繁检查
8. 技能冷却时间使用倒计时显示，按钮禁用状态
9. 游戏循环使用requestAnimationFrame实现60FPS流畅运行
10. 碰撞检测：迷宫模式使用坐标精确匹配，自由模式使用矩形碰撞检测
```
...
```
优化游戏体验和修复问题：
1. 修复迷宫死路问题，使用递归回溯算法确保迷宫连通性
2. 修复刺的位置，让刺替换墙而不是挡住路（检查walls[y][x] === 1）
3. 优化按钮布局，避免遮挡游戏画面，按钮放在画布左右两侧
4. 添加暂停功能，按ESC或空格键暂停/继续
5. 添加难度选择，影响鬼的生成速度、最大数量和移动速度
6. 完善成就检查逻辑，避免重复解锁
7. 优化粒子系统性能，自动清理过期粒子
8. 添加游戏统计面板，实时显示总游戏数、总时长等
9. 添加成就面板，显示所有成就的解锁状态
10. 优化UI响应式布局，适配不同屏幕尺寸
```
...
完善游戏功能：
1. 添加主菜单，包含难度选择、游戏说明
2. 添加游戏结束界面，显示详细统计和成就解锁情况
3. 添加暂停界面，可以继续游戏或返回主菜单
4. 完善成就系统，包含9个不同类型的成就
5. 添加游戏统计，记录总游戏数、总时长、总分数等
6. 添加粒子效果，收集物品和清除鬼时产生特效
7. 添加道具效果提示，使用技能时屏幕中央显示动画
8. 添加成就通知，解锁成就时显示弹窗动画
9. 数据持久化，所有数据保存在localStorage
10. 优化代码结构，添加清晰的注释和函数说明
```
